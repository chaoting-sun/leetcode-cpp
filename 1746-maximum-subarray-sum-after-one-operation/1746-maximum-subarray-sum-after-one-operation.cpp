...
public:
    ...
    //     vector<int> leftEnd(n), rightStart(n);
    //     int ans = INT_MIN;
        
    //     leftEnd[0] = nums[0];
    //     for (int i = 1; i < n; i++) {
    //         leftEnd[i] = max(leftEnd[i - 1] + nums[i], nums[i]);
    //     }

    //     rightStart[n - 1] = nums[n - 1];
    //     for (int i = n - 2; i >= 0; i--) {
    //         rightStart[i] = max(rightStart[i + 1] + nums[i], nums[i]);
    //     }

    //     for (int i = 0; i < n; i++) {
    //         int curr = nums[i] * nums[i];
    //         if (i > 0) curr = max(curr, curr + leftEnd[i - 1]);
    //         if (i < n - 1) curr = max(curr, curr + rightStart[i + 1]);
    //         ans = max(ans, curr);
    //     }
