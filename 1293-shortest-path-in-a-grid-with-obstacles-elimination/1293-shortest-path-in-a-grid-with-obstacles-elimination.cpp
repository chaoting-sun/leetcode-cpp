...
public:
    int shortestPath(vector<vector<int>>& grid, int k) {
        ...
        vector<vector<int>> maxRemainingObstacles(m, vector<int>(n, -1));
        queue<pair<int,int>> q; // distance, current obstacles
        q.push({ 0, k });
        maxRemainingObstacles[0][0] = k;
        int steps = 0;
        ...
        while (!q.empty()) {
            ...
            while (size--) {
                auto [p, remainingObstacles] = q.front();
                q.pop();
                int x = p / n;
                int y = p % n;
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (newRemainingObstacles < 0) continue; // invalid move
                    if (maxRemainingObstacles[newX][newY] >= newRemainingObstacles) continue; // already has a better path

                    maxRemainingObstacles[newX][newY] = newRemainingObstacles;
                    q.push({ newX * n + newY, newRemainingObstacles });
                }
            }
            steps++;
        }
